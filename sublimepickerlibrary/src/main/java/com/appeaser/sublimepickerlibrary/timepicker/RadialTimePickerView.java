...
//    private static final int[] MINUTES_NUMBERS = {0, 15, 30,45};
    private static final int[] MINUTES_NUMBERS = {0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55};

    private static int MIN_COUNT=MINUTES_NUMBERS.length;

    private static final int HOURS_IN_CIRCLE = 12;
    //    private static final int MINUTES_IN_CIRCLE = 4;
...
//    private static final float[] SIN_30 = new float[NUM_POSITIONS];
    private static final ArrayList<SinCosPair> hourDegrees=new ArrayList<>();
    private static final ArrayList<SinCosPair> minDegrees=new ArrayList<>();

//    private static final float[] MIN_COS = new float[MINUTES_IN_CIRCLE];
//    private static final float[] MIN_SIN = new float[MINUTES_IN_CIRCLE];
    ...
    static {
        ...
        for (int i = 0; i < NUM_POSITIONS; i++) {
            ...
            pair.cos = Math.cos(angle);
            pair.sin =  Math.sin(angle);
            angle += increment;
            hourDegrees.add(pair);
        }
        initMinSinCos();
    }

    private static void initMinSinCos() {
        final double increment = 2.0 * Math.PI /MIN_COUNT;
        double angle = Math.PI / 2.0;
    ...
    private final String[] mHours12Texts = new String[HOURS_IN_CIRCLE];
    private final String[] mOuterHours24Texts = new String[12];
    private final String[] mInnerHours24Texts = new String[12];
    private final String[] mMinutesTexts = new String[MIN_COUNT];
    ...
    public void setSelectorDotColor(@ColorInt int color){
        mSelectorDotColor=color;
        invalidate();
    }

    private final Paint mPaintBackground = new Paint();
    public void setNumBackgroundColor(@ColorInt int color){
    ...
    private final float[] minuteDegreeX = new float[MIN_COUNT];
    private final float[] minuteDegreeY = new float[MIN_COUNT];
...
//        Log.i("Hill->","setCurrentMinuteInternal degree->"+mSelectionDegrees[MINUTES]);
        invalidate();
...
//        return MINUTES_NUMBERS[degrees / DEGREES_FOR_ONE_MINUTE];
    ...
    private void drawMinutes(Canvas canvas, float alphaMod) {
        ...
        if (minutesAlpha > 0) {
            ...
            drawTextElements(canvas, mTextSize[MINUTES], mTypeface, mTextColor[MINUTES],
                    mMinutesText, minuteDegreeX, minuteDegreeY, mPaint[MINUTES],
    ...
    private void calculatePositionsHours() {
        ...
        calculatePositions(mPaint[HOURS], numbersRadius, mXCenter, mYCenter,
                mTextSize[HOURS], hourDegrees,mOuterTextX[HOURS], mOuterTextY[HOURS]);
        ...
        if (mIs24HourMode) {
            ...
            calculatePositions(mPaint[HOURS], innerNumbersRadius, mXCenter, mYCenter,
                    mTextSize[HOURS_INNER], hourDegrees,mInnerTextX, mInnerTextY);
        }
    }
    ...
    private void calculatePositionsMinutes() {
        ...
        calculatePositions(mPaint[MINUTES], numbersRadius, mXCenter, mYCenter,
                mTextSize[MINUTES], minDegrees,minuteDegreeX, minuteDegreeY);
    }
    ...
    private static void calculatePositions(Paint paint, float radius, float xCenter, float yCenter,
                                           float textSize,ArrayList<SinCosPair> degrees ,float[] x, float[] y) {
        // Adjust yCenter to account for the text's baseline.
        paint.setTextSize(textSize);
        yCenter -= (paint.descent() + paint.ascent()) / 2;

        for (int i = 0; i < degrees.size(); i++) {
            x[i] = xCenter - radius * (float) degrees.get(i).cos;
            y[i] = yCenter - radius * (float) degrees.get(i).sin;
        }
    }
...
//        Log.w("Hill->","drawTextElements degree"+activatedDegrees);

        // The activated index can touch a range of elements.
        final float activatedIndex = activatedDegrees / (360.0f / texts.length);
        final int activatedFloor = (int) activatedIndex;
        final int activatedCeil = ((int) Math.ceil(activatedIndex)) % texts.length;
...
//            Log.w("Hill->","degree->"+degrees+ " snapDegree->"+snapDegrees+" valueChange->"+valueChanged);
//            Log.w("Hill->","newValue->"+newValue);
        }
